...
function addBox(hex, container) {
    $('#colors').prepend("<div class='item' style='background-color: " + color + ";'><div>");
    //var newFavColor = document.createElement('div');
    //newFavColor.setAttribute('style', 'background-color:${hex}');
    //newFavColor.setAttribute('class','color_blocks');
    //container.appendChild(newFavColor);
...
//3.Write events handlers such that whenever any item in the favorite colors is clicked or hovered, the color gets displayed in the preview div
    $(document).on('mouseover','.item', function() {
        hoveredColor = $(this).css('background-color');
        setPreviewColor(hoveredColor);
